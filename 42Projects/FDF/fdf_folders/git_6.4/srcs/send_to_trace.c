...
/*   Updated: 2017/02/07 18:24:54 by tjacquin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		stock_create(fdf_t *stk)
{
	...
	while (y < (stk->ymax - 1))
	{
		...
		while (x < (stk->xmax - 1))
		{
			matricial(stk, (x + 1), (y + 1), (stk->map[y][x] * stk->hi), i++);
			x++;
		}
		y++;
	}
...
void		trace_vertical(fdf_t *stk)
{
	...
	while (y++ < (stk->ymax - 1))
	{
		...
		while (x++ < stk->xmax)
		{
			stk->x1 = stk->x_p[i] * stk->zoom + point + stk->decal_x;
			stk->x2 = stk->x_p[i + (stk->xmax - 1)] * stk->zoom + point + stk->decal_x;
			stk->y1 = stk->y_p[i] * stk->zoom + point + stk->decal_y;
			stk->y2 = stk->y_p[i++ + (stk->xmax - 1)] * stk->zoom + point + stk->decal_y;
			segment_tracer(stk);
		}
	}
}
...
void		trace_horizontal(fdf_t *stk)
{
	...
	while (y++ < stk->ymax)
	{
		...
		{
			stk->x1 = stk->x_p[i] * stk->zoom + point + stk->decal_x;
			stk->x2 = stk->x_p[i + 1] * stk->zoom + point + stk->decal_x;
			stk->y1 = stk->y_p[i] * stk->zoom + point + stk->decal_y;
			stk->y2 = stk->y_p[i++ + 1] * stk->zoom + point + stk->decal_y;
			segment_tracer(stk);
		}
		i++;
	}
