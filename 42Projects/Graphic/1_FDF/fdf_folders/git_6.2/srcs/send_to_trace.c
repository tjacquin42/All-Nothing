...
/*   Updated: 2017/02/06 22:26:01 by tjacquin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		matricial(fdf_t *stk, int x, int y, int z, int i)
{
	...
	x = cos(d_r(stk->ry)) * stk->x_p[i] + sin(d_r(stk->ry)) * stk->z_p[i];
	y = stk->y_p[i];
	z = -(sin(d_r(stk->ry))) * stk->x_p[i] + cos(d_r(stk->ry)) * stk->z_p[i];
	stk->x_p[i] = cos(d_r(stk->rz)) * x - sin(d_r(stk->rz)) * y;
	stk->y_p[i] = sin(d_r(stk->rz)) * x + cos(d_r(stk->rz)) * y;
	stk->z_p[i] = z;
}
...
void		stock_create(fdf_t *stk)
{
	...
	i = 0;
	y = 0;
	if (!(stk->x_p = (int *)malloc(sizeof(int) * (stk->xmax * stk->ymax))))
		return ;
	if (!(stk->y_p = (int *)malloc(sizeof(int) * (stk->xmax * stk->ymax))))
		return ;
	if (!(stk->z_p = (int *)malloc(sizeof(int) * (stk->xmax * stk->ymax))))
		return ;
	while (y < (stk->ymax - 1))
	{
		x = 0;
